...
function onOpen() {
  ...
  writeComment("DC - Generated using modified PP compatible with linuxCNC automated toolchange.");
  ...
  if (insertToolCall || retracted || (!isFirstSection() && getPreviousSection().isMultiAxis())) {
    ...
    if (!machineConfiguration.isHeadConfiguration()) {
      writeBlock(
        ...
        gMotionModal.format(0), yOutput.format(initialPosition.y)
        );
